...
const page = MOBILE_DEGRADE_PAGES.P42
...
async function load() {
  ...
  try {
    ...
    error.value = e instanceof Error ? e.message : '无法装配访前包降级壳层'
  } finally {
    loading.value = false
  }
}
...
<template>
  ...
  >
    <template #toolbar>
      ...
      />
      <RouterLink class="desktop-link" data-testid="p42-desktop-pack" to="/engagement/previsit/pack">
        查看桌面访前包（只读）
      </RouterLink>
    </template>
    <PageState :status="status" :error="error" :idle-description="page.idleDescription" @retry="load">
      <p class="empty" data-testid="p42-mobile-previsit-empty">{{ page.emptyText }}</p>
    </PageState>
  </MobileDegradeFrame>
</template>
