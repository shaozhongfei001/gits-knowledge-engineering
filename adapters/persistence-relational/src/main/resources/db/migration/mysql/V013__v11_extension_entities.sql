...
CREATE TABLE IF NOT EXISTS evidence_version_link (
    ...
    INDEX idx_evl_evidence (evidence_id),
    INDEX idx_evl_previous (previous_version_id)
...
CREATE TABLE IF NOT EXISTS claim_lifecycle_event (
    ...
    INDEX idx_cle_claim (claim_id),
    INDEX idx_cle_actor (actor_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS interaction_extension (
    ...
    INDEX idx_ie_consent (recording_consent_id),
    INDEX idx_ie_kyc (kyc_gap_profile_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
